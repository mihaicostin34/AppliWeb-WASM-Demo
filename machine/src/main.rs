...
async fn main()  {
    ...
    tokio::spawn(async move{
        ...
        loop{
            ...
            let (name, ip) = mock_host_data.get(random as usize).unwrap();
            ...
            let res = client.post("http://localhost:8080/serveur/ajoutsysinfo")
                .form(&params)
                .send()
                .await.unwrap();

            println!("Sent request {}!", res.status().as_str());
